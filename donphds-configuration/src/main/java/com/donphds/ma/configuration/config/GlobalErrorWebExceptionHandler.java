        ...
        dataBuffer =
            bufferFactory.wrap(
                om.writeValueAsBytes(
                    Maps.immutableEntry("msg", ((HttpStatusCodeException) ex).getStatusText())));
        return response.writeWith(Mono.just(dataBuffer));
      } else if (ex instanceof IOException) {
        response.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR);
        dataBuffer =
            bufferFactory.wrap(om.writeValueAsBytes(Maps.immutableEntry("msg", "网络发生了一点小抖动~")));
      } else {
        response.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR);
        dataBuffer = bufferFactory.wrap(om.writeValueAsBytes(Maps.immutableEntry("msg", "未知的异常")));
      }
